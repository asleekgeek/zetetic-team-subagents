...
**Optional MCP server: `automatised-pipeline`** (from [`ai-automatised-pipeline`](https://github.com/cdeust/ai-automatised-pipeline)). When configured, the reviewer can ground every verdict in graph-level evidence instead of file-local inspection.
...
```bash
MEMORY_AGENT_ID=code-reviewer tools/memory-tool.sh create   /memories/code-reviewer/checkpoint.md "$(cat <<'CHECKPOINT'
## Checkpoint <ISO-date>
...
MEMORY_AGENT_ID=code-reviewer tools/memory-tool.sh view /memories/code-reviewer/checkpoint.md
```
...
- Cross-session notes (rejected approaches, confirmed constraints) go in a separate `/memories/code-reviewer/notes.md`.
