...
<memory>
**Your memory topic is `genius-carnot`.**
...
- Your declared path is **`/memories/genius/carnot/`** — this is your namespace.
...
|---|---|---|---|
| `view` | `tools/memory-tool.sh view /memories/genius/carnot/` | Exact bytes or directory listing. Deterministic. | Session start — always. Also for known file paths. |
...
File naming convention: `/memories/genius/carnot/<topic>.md` — one file per reasoning domain.
...
```bash
MEMORY_AGENT_ID=genius-carnot tools/memory-tool.sh create   /memories/genius/checkpoint.md "$(cat <<'CHECKPOINT'
## Checkpoint <ISO-date>
...
MEMORY_AGENT_ID=genius-carnot tools/memory-tool.sh view /memories/genius/
# Then load the checkpoint:
MEMORY_AGENT_ID=genius-carnot tools/memory-tool.sh view /memories/genius/checkpoint.md
```
