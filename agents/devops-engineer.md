...
**Optional MCP server: `automatised-pipeline`** (from [`ai-automatised-pipeline`](https://github.com/cdeust/ai-automatised-pipeline)). Less central for DevOps than for code-reasoning roles, but useful when CI/CD wiring depends on the application graph.
