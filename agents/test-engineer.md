...
**Optional MCP server: `automatised-pipeline`** (from [`ai-automatised-pipeline`](https://github.com/cdeust/ai-automatised-pipeline)). The graph tells you what to test — manual coverage estimation cannot.
...
```bash
MEMORY_AGENT_ID=test-engineer tools/memory-tool.sh create   /memories/test-engineer/checkpoint.md "$(cat <<'CHECKPOINT'
## Checkpoint <ISO-date>
...
MEMORY_AGENT_ID=test-engineer tools/memory-tool.sh view /memories/test-engineer/checkpoint.md
```
...
- Cross-session notes (rejected approaches, confirmed constraints) go in a separate `/memories/test-engineer/notes.md`.
