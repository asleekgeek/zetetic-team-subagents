...
scan_file() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if printf '%s' "$line" | grep -qE "$CONTRAST_RE"; then
      echo "$f:$lineno: CONTRAST: $(printf '%s' "$line" | grep -oE "$CONTRAST_RE" | head -1 | cut -c1-60) (redaction §9/§35)"
      findings=$((findings + 1))
    fi
    if printf '%s' "$line" | grep -qE "$SETUP_RE"; then
      echo "$f:$lineno: SETUP: $(printf '%s' "$line" | grep -oE "$SETUP_RE" | head -1) (redaction §27-31)"
      findings=$((findings + 1))
    fi
    if printf '%s' "$line" | grep -qE "$PUFF_RE"; then
      echo "$f:$lineno: PUFFERY: $(printf '%s' "$line" | grep -oE "$PUFF_RE" | head -1) (redaction §1/§4/§8/§20-22)"
      findings=$((findings + 1))
    fi
  done < "$f"
}
