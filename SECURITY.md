...
- **`tools/*.sh` and `tools/*.py`** run as part of pre-commit / pre-push gates
  (the zetetic checker, redaction checker, craftsmanship checker, mutation
  gates).
...
execution — the payload does not even need to be compiled. That is the threat
model the assurance below is built for.
...
- **Signed provenance** — the release bundle
  ...
  is a source-only plugin there is no binary to sign, so signed releases/tags
  over the bundle are the achievable attestation. Verify a download:
...
- **Continuous analysis** — shellcheck over `hooks/`+`tools/` (`shellcheck.yml`,
  error-severity hard gate), CodeQL for the python hooks/tools (`codeql.yml`),
  and OpenSSF Scorecard (`scorecard.yml`), all on a schedule. The Scorecard
  number is a recorded baseline, not a badge.
