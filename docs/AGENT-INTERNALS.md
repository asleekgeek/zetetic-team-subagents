...
Every agent ships as a single Markdown file with YAML frontmatter. Slim frontmatter keeps cumulative description tokens across all 116 agents under Claude Code's startup cap (~12.6k tokens, was 28k in v2.12.0). Rich routing detail lives in body sections, loaded only when the agent is invoked.
